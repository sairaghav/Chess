...
class Player(object):
    ...
    def get_user_input(self):
        try:
            from_x_pos,from_y_pos = self.board.convert_to_coordinates(input('Enter the from position: '))
            ...
            to_x_pos,to_y_pos= self.board.convert_to_coordinates(input('Enter the to position: '))
    ...
    def start_single_player(self):
        ...
        while self.end_of_game == 0:
            try:
                ...
                if color_of_player.lower()[0] == self.board.current_player:
                    piece_to_move = self.board.get_piece(from_x_pos,from_y_pos)
                    cut_piece,moved_piece = self.board.make_move(piece_to_move,to_x_pos,to_y_pos)
